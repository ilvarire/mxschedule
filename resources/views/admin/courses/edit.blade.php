    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Update details for {{ $course->code }}</p>
            </div>
        </div>
    </x-slot>
                ...
                <form action="{{ route('admin.courses.update', $course) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="code" name="code" type="text" class="mt-1 block w-full uppercase" :value="old('code', $course->code)" placeholder="e.g. CSE301" required />
                            <x-input-error :messages="$errors->get('code')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="credit_units" value="Credit Units" />
                            <x-text-input id="credit_units" name="credit_units" type="number" min="1" max="10" class="mt-1 block w-full" :value="old('credit_units', $course->credit_units)" placeholder="e.g. 3" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title', $course->title)" placeholder="e.g. Introduction to Software Engineering" required />
